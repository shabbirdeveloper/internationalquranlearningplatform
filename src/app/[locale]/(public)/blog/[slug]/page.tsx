...
const labels = { en: { back: "Back to the journal", minutes: "min read", by: "By" }, ur: { back: "جرنل پر واپس جائیں", minutes: "منٹ مطالعہ", by: "تحریر" }, ar: { back: "العودة إلى المجلة", minutes: "دقائق قراءة", by: "بقلم" }, fa: { back: "بازگشت به مجله", minutes: "دقیقه مطالعه", by: "نویسنده" } } as const;
    ...
    <article>
      <header className="public-page-hero-pattern border-b border-sidebar-border text-sidebar-foreground">
        <div className="mx-auto max-w-5xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
          <Button variant="ghost" className="-ms-3 mb-7 text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-foreground" asChild><Link href={`/${locale}/blog`}><ArrowLeftIcon className="rtl:rotate-180" />{copy.back}</Link></Button>
          <div className="flex flex-wrap items-center gap-3"><Badge className="bg-gold text-sidebar hover:bg-gold">{post.category}</Badge><span className="inline-flex items-center gap-1.5 text-sm text-sidebar-foreground/65"><Clock3Icon className="size-4" />{post.reading_time_minutes} {copy.minutes}</span></div>
          ...
          <p className="mt-7 text-sm font-medium text-gold">{copy.by} {post.author_name}</p>
        </div>
      </header>
