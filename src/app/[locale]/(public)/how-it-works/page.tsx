...
function Journey({ title, steps, icon: Icon }: { title: string; steps: string[]; icon: typeof GraduationCapIcon }) { return <Card data-premium-interactive="true" data-premium-hover="surface"><CardHeader><span data-hover-icon className="flex size-11 items-center justify-center rounded-xl bg-primary text-primary-foreground"><Icon className="size-5"/></span><CardTitle className="mt-4 text-2xl">{title}</CardTitle></CardHeader><CardContent><ol className="space-y-2">{steps.map((step,index) => <li key={step} data-premium-hover="row" className="-mx-3 flex gap-4 rounded-xl px-3 py-2"><span data-hover-number className="flex size-8 shrink-0 items-center justify-center rounded-full border border-primary/20 font-semibold text-primary">{index + 1}</span><span className="pt-1 leading-6">{step}</span></li>)}</ol></CardContent></Card>; }
