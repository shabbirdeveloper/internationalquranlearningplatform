...
function PostMeta({ post }: { post: BlogPost }) {
  return <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><Badge variant="secondary">{post.category}</Badge><span className="inline-flex items-center gap-1"><Clock3Icon className="size-3.5" />{post.reading_time_minutes} min read</span></div>;
}
    ...
    <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-20 lg:px-8">
      <article className="group grid overflow-hidden rounded-[2rem] border border-border/70 bg-card shadow-[0_25px_80px_-55px_oklch(0.15_0.04_248)] lg:grid-cols-[1.15fr_0.85fr]">
        <div className="relative min-h-72 overflow-hidden lg:min-h-[31rem]">
          ...
          <Badge className="absolute start-5 top-5 bg-gold text-sidebar hover:bg-gold">Featured insight</Badge>
        </div>
        <div className="flex flex-col justify-center p-7 sm:p-10 lg:p-12">
          ...
          <p className="mt-6 text-sm font-medium text-primary">By {featured.author_name}</p>
          <Button asChild className="mt-7 w-fit">
            <Link href={`/${locale}/blog/${featured.slug}`}>Read the story <ArrowRightIcon className="rtl:rotate-180" /></Link>
          </Button>
        </div>
      </article>
      ...
      {remaining.length ? <div className="mt-14 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {remaining.map((post) => <article key={post.id} className="public-interactive-card group flex h-full flex-col overflow-hidden rounded-3xl border border-border/70 bg-card shadow-sm">
          ...
          <div className="flex flex-1 flex-col p-6">
            ...
            <Link href={`/${locale}/blog/${post.slug}`} className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-primary">Read article <ArrowRightIcon className="size-4 rtl:rotate-180" /></Link>
          </div>
        </article>)}
      </div> : null}
    </section>
